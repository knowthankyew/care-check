...
describe('LetterGeneratorEngine', () => {
  ...
  it('includes Notice & Self-Advocacy Disclaimer in copied markdown output across all letter types', () => {
    ...
    expect(doc501r.markdownContent).toContain('does not constitute formal legal advice');
    expect(doc501r.markdownContent).toContain('does not provide legal representation');
  ...
  it('incorporates exact non-waivable UPL disclaimer string in generated markdown and HTML', () => {
    const doc = generateDisputeLetter(samplePayload);
    const expectedUPL = 'This document was generated by an automated open-source software tool for informational advocacy purposes and does not constitute formal legal advice.';
    expect(doc.markdownContent).toContain(expectedUPL);
    expect(doc.htmlContent).toContain(expectedUPL);
  });

  it('correctly attributes 26 U.S.C. § 501(r)(6) to original hospital and reserves 15 U.S.C. § 1692g for third-party collection agencies', () => {
    const auditDoc = generateDisputeLetter({
      ...samplePayload,
      letterType: 'PRICE_TRANSPARENCY_AUDIT_DISPUTE',
    });

    // 26 U.S.C. § 501(r)(6) applies to the hospital billing department
    expect(auditDoc.markdownContent).toContain('pursuant to 26 U.S.C. § 501(r)(6)');

    // 15 U.S.C. § 1692g specifically applies if referred to third-party collection agencies
    expect(auditDoc.markdownContent).toContain('referred or assigned to an external third-party collection agency');
    expect(auditDoc.markdownContent).toContain('Fair Debt Collection Practices Act (**15 U.S.C. § 1692g**)');

    // Statutory citations note third-party scope for FDCPA
    const fdcpaCitation = auditDoc.statutoryCitations.find((c) => c.includes('1692g'));
    expect(fdcpaCitation).toContain('Applicable to Third-Party Debt Collectors');
  });
});
