...
export function getPrivacyClaims(report: PrivacyAuditReport): PrivacyClaims {
  if (report.isLocalOnlyHonest) {
    return {
      ...
      disclaimerExecutionText: 'All medical bill evaluations execute 100% locally in your browser with zero remote network transmission.',
      footerTitle: '100% Local Air-Gapped Medical Bill Compliance & Charity Care Engine.',
      footerSubtext: 'Zero Telemetry • Zero Remote PHI/Document Egress • Section 501(r) Reality Engine',
  ...
  return {
    ...
    footerTitle: 'Enterprise Medical Bill Compliance & Charity Care Engine (OTLP Telemetry Active).',
    footerSubtext: 'Enterprise Telemetry Mode • Operational Metadata Export Active • Medical Bills Air-Gapped',
      ...
      await fetch(this.config.otlpEndpoint, {
        ...
        body: JSON.stringify({
          resourceSpans: [
            {
              resource: {
                attributes: [{ key: 'service.name', value: { stringValue: 'care-check' } }],
              },
              scopeSpans: [
                {
                  spans: [
